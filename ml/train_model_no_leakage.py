...
MODEL_PATH = str(PROJECT_ROOT / "models" / "rf_no_leakage_v1")
RESULTS_PATH = PROJECT_ROOT / "report" / "model_results_no_leakage.md"
...
LEAKAGE_EXCLUDED_FEATURES = {
    ...
    # Reviewer-behavior features likely used directly or indirectly in weak labeling.
    "rev_total_reviews",
    "rev_max_per_day",
    "rev_rating_stddev",
    "rev_days_active",
    ...
    # Cross-feature likely too close to reviewer behavior heuristic.
    "xf_rating_vs_reviewer",

    # This may encode an explicit suspiciousness rule.
...
def main():
    spark = (
        ...
        .appName("TrainFakeReviewRandomForestNoLeakage")
        .master("local[*]")
    ...
    feature_cols = [
        ...
        if field.name not in LEAKAGE_EXCLUDED_FEATURES
    ...
    print("\nUsing leakage-controlled features:")
    ...
    print("\nTraining leakage-controlled Random Forest...")
    ...
    excluded_md = "\n".join(
        f"- `{name}`"
        for name in sorted(LEAKAGE_EXCLUDED_FEATURES)
        if name not in {"review_id", "product_category", "is_likely_fraud", "label"}
    )

    RESULTS_PATH.write_text(
        f"""# Leakage-Controlled Model Results
